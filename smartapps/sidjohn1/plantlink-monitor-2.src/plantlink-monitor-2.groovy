...
def pageInfo() {
	return dynamicPage(name: "pageInfo", install: true, uninstall: true) {
        ...
        def verb = ""
			...
			try {
				if (it.plantStatus.contains("Needs")) {
					verb = " "
				}
				else {
					verb = " is "
				}
				plantlist += "$it.displayName$verb${it.plantStatus.toLowerCase()}\n"
			} catch (e) {
...
def sendStatus() {
	...
	settings.senors.each() {
		try {
			...
			log.trace "${it.displayName}$verb${it.plantStatus.toLowerCase()}"
			sendEvent(linkText:app.label, name:"${it.displayName}", value:"${it.plantStatus}", descriptionText:"${it.displayName}$verb${it.plantStatus.toLowerCase()}", eventType:"SOLUTION_EVENT", displayed: true)
			if (it.plantStatus == "Too Wet" && settings.sendTooWet == true) {
				send("${it.displayName}$verb${it.plantStatus.toLowerCase()}")
			}
			if ((it.plantStatus == "Too Dry" || it.plantStatus == "Needs Water") && settings.sendTooDry == true) {
				send("${it.displayName}$verb${it.plantStatus.toLowerCase()}")
			}
		}
		catch (e) {
