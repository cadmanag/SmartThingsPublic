 ...
 */
definition(
    name: "Power Switch on Energy Usage",
    namespace: "smartthings",
    author: "SmartThings",
		description: "Turn things on when something else uses power.",
...
def meterHandler(evt) {
    ...
    if (meterValue > thresholdValue) {
	    log.debug "${meter} reported energy consumption above ${threshold}. Turning on (v)switches."
    	switches.on()
    }
    if (meterValue < thresholdValue) {
	    log.debug "${meter} reported energy consumption below ${threshold}. Turning off (v)switches."
    	switches.off()
    }
}