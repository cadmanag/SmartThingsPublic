...
private void api(method, args, success = {}) {
	...
	if (isTokenExpired()) {
		...
		def buffer_time_refresh=4  // set a 4 min. buffer time before re-attempting refresh 
		...
		if ((state?.lastRefreshTimestamp) && (state?.lastRefreshTimestamp > time_check_for_refresh)) {
			if (settings.trace) {
				log.debug "api>thermostatId = ${thermostatId},time_check_for_refresh (${time_check_for_refresh}) < state.lastRefreshTimestamp (${state?.lastRefreshTimestamp}), not refreshing tokens..."
				sendEvent name: "verboseTrace", value:
					"api>thermostatId = ${thermostatId},time_check_for_refresh (${time_check_for_refresh} < state.lastRefreshTimestamp (${state?.lastRefreshTimestamp}), not refreshing tokens..."
			}
			return 
		}    
		if (!refresh_tokens()) {
...
def refreshLocalAuthToken() {	
	...
	if ((state?.lastRefreshTimestamp) && (state?.lastRefreshTimestamp > time_check_for_refresh)) {
		if (settings.trace) {
			log.debug "refreshLocalAuthToken>thermostatId = ${thermostatId},time_check_for_refresh (${time_check_for_refresh}) < state.lastRefreshTimestamp (${state?.lastRefreshTimestamp}), not refreshing tokens..."
			sendEvent name: "verboseTrace", value:
				"refreshLocalAuthToken>thermostatId = ${thermostatId},time_check_for_refresh (${time_check_for_refresh} < state.lastRefreshTimestamp (${state?.lastRefreshTimestamp}), not refreshing tokens..."
		}
		return false
	}
	...
	try {
		...
		state?.lastPollTimestamp= now() // introduce a poll delay
		log.error "refreshLocalAuthToken>caught exception while refreshing auth token: " + e
		sendEvent name: "verboseTrace", value:
			"refreshLocalAuthToken>caught exception while refreshing auth token: " + e            
	}
...
//	if device created by initialSetup (Service Manager)
	...
	if (data.auth?.thermostatId) {
		...
		try {
			if (settings.trace) {
				log.debug "refresh_tokens>about to call parent.refreshAuthToken()"
			}
			if (parent.isTokenExpired()) {
				parent.refreshAuthToken(data.auth.refresh_token)
			}   
			newAuthTokens= parent.getAuthTokens()            
			if (settings.trace) {
				log.debug "refresh_tokens>new authTokens= ${newAuthTokens} from parent"
			}
			if (!refreshChildTokens(newAuthTokens)) { // try to refresh the device tokens using the parent tokens
				if (settings.trace) {
					sendEvent name: "verboseTrace", value:
						"refresh_tokens>warning: child tokens not refreshed after refreshChildTokens() call"
					log.warn "refresh_tokens>warning: child tokens not refreshed after refreshChildTokens() call"
				}
			...
			state?.lastPollTimestamp = now()  // Introduce a poll delay 
			state?.lastRefreshTimestamp = now()  // Introduce a refresh delay 
			log.error "refresh_tokens>exception $e"
			sendEvent name: "verboseTrace", value:
				"refresh_tokens>exception $e"
			state.exceptionCount = state.exceptionCount +1     
			return false
		}
		return true        
...
private def isTokenExpired() {
	...
	if (settings.trace) {
		...
		log.debug "isTokenExpired>state= $state"
		sendEvent name: "verboseTrace", value:
			"isTokenExpired>expire in ${authExpTimeInMin.intValue()} minutes"
	}            
    if (authExpTimeInMin <0) {
 		log.warn "isTokenExpired>auth token buffer time ${buffer_time_expiration} expired, countdown is ${authExpTimeInMin.intValue()} minutes, need to refresh tokens now! about to call refreshLocalAuthToken()..."
		sendEvent name: "verboseTrace", value: "isTokenExpired>state= $state"
		sendEvent name: "verboseTrace", value: "isTokenExpired>data.auth= $data.auth"
		sendEvent name: "verboseTrace", value:
			"isTokenExpired>auth token buffer time ${buffer_time_expiration} expired, countdown is ${authExpTimeInMin.intValue()} minutes, need to refresh tokens now! about to call refreshLocalAuthToken()..."
	}    
    if (authExpTimeInMin < (0-buffer_time_expiration)) {
		log.error "isTokenExpired>refreshing tokens is is now at risk (${authExpTimeInMin}),exception count may increase!"
		sendEvent name: "verboseTrace", value:
			"isTokenExpired>refreshing tokens is now at risk (${authExpTimeInMin}),exception count may increase!"
	}    
