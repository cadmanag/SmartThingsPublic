...
metadata {
    definition (name: "ZigBee Switch Power - Custom", namespace: "smartthings", author: "SmartThings") {
        capability "Actuator"
    ...
    tiles(scale: 2) {
        ...
        main "switch"
        details(["switch", "refresh", "statusText"])
    }
}
    ...
    if (resultMap) {
        ...
        if (resultMap.type == "update") {
            ...
            if (device.getDataValue("manufacturer") != "OSRAM") {       //OSRAM devices do not reliably update power
                powerValue = (resultMap.value as Integer)/1            //TODO: The divisor value needs to be set as part of configuration
