...
metadata {
    definition (name: "ZigBee White Color Temperature Bulb - Custom", namespace: "smartthings", author: "SmartThings") {
    ...
    tiles(scale: 2) {
        ...
        valueTile("statusText", "statusText", inactiveLabel: false, decoration: "flat", width: 4, height: 2) {
			state "statusText", label:'${currentValue}', backgroundColor:"#ffffff"
		}  

        main(["switch"])
        details(["switch", "colorTempSliderControl", "colorTemp", "refresh", "statusText"])
    }
}
...
def configure() {
    ...
    zigbee.onOffConfig() + zigbee.levelConfig() + zigbee.colorTemperatureConfig() + zigbee.onOffRefresh() + zigbee.levelRefresh() + zigbee.colorTemperatureRefresh() + zigbee.off()
}
