...
metadata {
	...
	definition (name: "PlantLink Direct Sensor", namespace: "KristopherKubicki", author: "kristopher@acm.org") {
        ...
        attribute "wpercent","number"
        attribute "vdrop","number"

		fingerprint profileId: "0104", inClusters: "0000,0001,0003,0B04"
	}
    
    preferences {
		input "soilType", "enum", title: "Soil Type", options: ["Sand","Loamy Sand","Sandy Loam","Silt","Silty Loam","Silty Sandy Loam","Sandy Clay Loam","Loam","Clay Loam","Silty Clay Loam","Silty Clay","Sandy Clay"], required: false, displayDuringSetup: true, defaultValue: "Loam"
	...
	tiles {
        ...
        valueTile("wpercent", "device.wpercent", inactiveLabel: false, decoration: "flat") {
            state "wpercent", label:'${currentValue}% Water Content', unit:""
        }

        main "humidity"
        details(["humidity","soilType","battery", "water","status","voltage","resistivity","wpercent"])
	}
}
...
private calculateHumidity(value) {
    ...
    log.debug "RAW: $value"
    def vdrop = Integer.parseInt(value, 16)
    log.debug "VoltageDrop: $vdrop"
    sendEvent(name: "vdrop", value: vdrop)
    // exponential curve fit to PlantLink data.  Potentially more accurate than Oso's interpretation
    def resistivity = (1500000 * 2.71828 ** (-0.000888 * vdrop)) as Integer
    ...
    def wpercent = 0.00
    wpercent = Math.round (percent * 100.00) / 100.00
    log.debug "WPERCENT: $wpercent"
    sendEvent(name: "wpercent", value: wpercent)
...
private calculateBattery(value) {
	def min = 2300
	def bpercent = (Integer.parseInt(value, 16) - min) / 10
   
	bpercent = Math.max(0.0, Math.min(bpercent, 100.0))
    if(bpercent < 5) {
    	sendEvent(name: "status", value: "Change Battery!")
    }
    def voltage = sprintf("%.2f",(4.073 + (bpercent / 100))/1.67)
    log.debug "BATTERY %: $bpercent"
...
private percentToWarn(percent) { 
    ...
    else if(soilType == "Silty Sandy Loam") { 
            moisturePercent = (percent - 19.75) / (38 - 17)
    }
...
log.debug "MPERCENT (Water potential): $moisturePercent"
